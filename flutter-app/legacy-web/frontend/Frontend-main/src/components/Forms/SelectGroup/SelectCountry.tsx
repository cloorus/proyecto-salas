...
// The properties that the `SelectCountry` component accepts are defined
interface SelectCountryProps {
  currentCountry?: number;            // Country currently selected
  setCountryId: (id: number) => void; // Function to update the selected country
}

// Functional component `SelectCountry`
const SelectCountry: React.FC<SelectCountryProps> = ({ currentCountry, setCountryId }) => {
  ...
  // useEffect that is executed when the current country or the data obtained changes
  useEffect(() => {
    if (currentCountry) {
      const country = data.find((item) => item.ID_Country === currentCountry); // Check that the current country exists
      if (country) { // If the current country exists, update the state of the selected option
  ...
  }, [currentCountry, data]); // Executed when `currentCountry` or `data` changes

  // Executed when changing the country in the select
  const handleChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    setCountryId(selectedId);         // Call the function to update the selected country
  };
    ...
    <div>
      ...
      <div className="relative z-20 bg-white dark:bg-form-input">
        ...
        >
          ...
          {/* Mapping the list of countries to create the select options */}
          {data.map((item) => (
            <option
              key={item.ID_Country}
