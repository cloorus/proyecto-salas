...
interface UpdateEngineProps {
	...
	updateEngine: Engine; // Engine to be updated
	setRefreshData: React.Dispatch<React.SetStateAction<boolean>>; // Function to trigger a refresh of the data after update
};

const UpdateEngineModal: React.FC<UpdateEngineProps> = ({
	handleCloseUpdateModal,
	updateEngine,
	setRefreshData
}) => {
	const [engineTypeId, setEngineTypeId] = useState<number>(updateEngine.ID_Type ? updateEngine.ID_Type : 0);

	// Function that handles the update process with formik and Yup to validate
	const formik = useFormik({
		initialValues: {
			Model: updateEngine.Model,
			Brand: updateEngine.Brand,
			Link: updateEngine.Link
		},
		...
		onSubmit: async (values) => {
			try {
				...
				const result = await putFetch(`engines/${updateEngine.ID_Engine}`, engine);

				// Handle error messages
				if (result.duplicatedModel) {
					return toast.error("Modelo duplicado", {
						autoClose: 3000,
						theme: "dark"
					})
				};
				...
				if (result.notEngineType) {
					return toast.error("Tipo de motor no seleccionado'", {
						autoClose: 3000,
						theme: "dark"
					})
				};
		...
		<div className="mt-20 fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
			<div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
				<div className="border-b border-stroke py-4 px-6.5 dark:border-strokedark flex justify-between items-center">
					<h3 className="font-medium text-black dark:text-white">Editar Motor</h3>
				...
				<form onSubmit={formik.handleSubmit}>
					<div className="p-6.5 scroll-smooth">
						<div className="mb-4.5 flex flex-col gap-6 xl:flex-row">
							...
							<SelectEngineType currentType={updateEngine.Type} setEngineTypeId={setEngineTypeId} />
						</div>
