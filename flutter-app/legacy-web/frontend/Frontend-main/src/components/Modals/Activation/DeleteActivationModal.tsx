...
const DeleteActivationModal: React.FC<DeleteActivationProps> = ({
	...
	const handleDeleteActivation = async () => {
		try {
			...
			const result = await deleteFetch(`activations/${deleteActivationId}`);
		...
		<div className="mt-20 fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
			<div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
				<div className="border-b border-stroke py-4 px-6.5 dark:border-strokedark flex justify-between items-center">
					<h3 className="font-medium text-black dark:text-white">Eliminar Activación</h3>
				...
				<div className="p-6.5">
					<p className="text-black dark:text-white">¿Estás seguro de que deseas eliminar esta activación?</p>
				</div>
				<div className="flex justify-center border-t border-stroke p-6.5 dark:border-strokedark">
					<button onClick={() => handleDeleteActivation()} type="button" className="rounded bg-danger py-2.5 px-6 text-base font-medium text-white shadow-button transition hover:opacity-80">
						Eliminar
					</button>
...
export default DeleteActivationModal; // Exporting the component 
