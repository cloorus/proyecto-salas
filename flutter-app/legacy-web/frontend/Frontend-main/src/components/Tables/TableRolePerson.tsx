...
import CreateRolePersonModal from '../Modals/RolePerson/CreateRolePersonModal';
import UpdateRolePersonModal from '../Modals/RolePerson/UpdateRolePersonModal';
import DeleteRolePersonModal from '../Modals/RolePerson/DeleteRolePersonModal';
import { getEndpoint, getCurrentUser, paginateData } from '../../utils/utils';

interface TableRolePersonProps {
  searchedData: string | null;
}

const TableRolePerson: React.FC<TableRolePersonProps> = ({ searchedData }) => {
  // State variables for managing  data, UI state, pagination, and modal visibility.
  const [data, setData] = useState<RolePerson[]>([]);
  const [deleteRolePersonId, setDeleteRolePersonId] = useState<number>();
  const [refreshData, setRefreshData] = useState<boolean>(true);
  const [showCreateModal, setShowCreateModal] = useState<boolean>(false);
  const [showEditModal, setShowUpdateModal] = useState<boolean>(false);
  const [showDeleteModal, setShowDeleteModal] = useState<boolean>(false);
  const [editRolePerson, setEditRolePerson] = useState<RolePerson | null>(null);

  //Current page of pagination
  const [currentPage, setCurrentPage] = useState<number>(1);

  // Define the endpoint for fetching role person data
  const endpoint = getEndpoint('role-persons');
 ...
 // Handle pagination of data
  ...
  useEffect(() => {
    // Function to fetch role person data from the API
    const fetchData = async () => {
      try {
        const result = await getFetch(`${endpoint}`);
        setData(result as RolePerson[]);
  ...
  const filteredData = useMemo(() => {
    ...
    return paginatedData.filter(rolePerson =>
      rolePerson.Identify.toLowerCase().includes(searchTerm) ||
      rolePerson.Name.toLowerCase().includes(searchTerm) ||
      rolePerson.Role.toLowerCase().includes(searchTerm)
    );
  ...
  const handleOpenUpdateModal = (RolePerson: RolePerson) => {
    setEditRolePerson(RolePerson);
    setShowUpdateModal(true);
  };

  const handleCloseUpdateModal = () => {
    setShowUpdateModal(false);
    setEditRolePerson(null);
  }

  const handleOpenDeleteModal = (id: number) => {
    setDeleteRolePersonId(id);
    ...
    <>
      ...
      <div className="rounded-sm border border-stroke bg-white px-5 pt-6 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
        <div className="flex justify-end">
          ...
          {(currentUser.ID_Role !== 5 && currentUser.ID_Role !== 6 && currentUser.ID_Role !== 4) && (
        ...
        {/* Assign (create) Role Modal*/}
        {showCreateModal && (
          <CreateRolePersonModal
        ...
        {/* Edit Assigned Role Modal*/}
        {showEditModal && editRolePerson && (
          <UpdateRolePersonModal
            data={data}
        ...
        {/* Delete Assigned Role Modal */}
        {showDeleteModal && (
          <DeleteRolePersonModal
            handleCloseDeleteModal={handleCloseDeleteModal}
            deleteRolePersonId={deleteRolePersonId}
            setRefreshData={setRefreshData}
                ...
                filteredData.length > 0 ? (
                  filteredData.map((item, key) => (
                    <tr key={key}>
                      ...
                      <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                        {/* Show update and delete buttons if the current user has the appropriate role */}
                        {(currentUser.ID_Role !== 5 && currentUser.ID_Role !== 6 && currentUser.ID_Role !== 4) && (
                          <div className="flex items-center space-x-3.5">
                            ...
                            <button onClick={() => { if (item.ID_Role_Person) handleOpenDeleteModal(item.ID_Role_Person) }} className="hover:text-primary">
                              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
...
export default TableRolePerson;
