...
import { getEndpoint, paginateData, dateFormatted } from '../../utils/utils';

interface TableValidationProps {
	searchedData: string | null;
}

const TableValidation: React.FC<TableValidationProps> = ({ searchedData }) => {
	...
	// Define the endpoint for fetching validations data
	const endpoint = getEndpoint('validations');
	...
	useEffect(() => {
		// Function to fetch validation data from the API
		const fetchData = async () => {
			try {
				const result = await getFetch(`${endpoint}`);
				setData(result as Validation[]);
	...
	const filteredData = useMemo(() => {
		...
		return paginatedData.filter(validation =>
			validation.Validated_Person.toLowerCase().includes(searchTerm) ||
			validation.Validator_Person.toLowerCase().includes(searchTerm) ||
			validation.Date_Validation.slice(0, 10).toLowerCase().includes(searchTerm)
		);
	...
	return (
		<>
			<Breadcrumb pageName="Validaciones" />
										...
										<tr key={key}>
											<td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
												<p className="text-black dark:text-white">
													{item.Name_Validated_Person} - {item.Identify_Validated_Person}
												</p>
											</td>
											<td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
												<p className="text-black dark:text-white">
													{item.Name_Validator_Person} - {item.Identify_Validator_Person}
												</p>
											</td>
											<td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
												<p className="text-black dark:text-white">
													{dateFormatted(item.Date_Validation.slice(0, 10), "cr")}
												</p>
											</td>
										</tr>
									))
