...
import CreateMembershipModal from '../Modals/Membership/CreateMembershipModal';
import DeleteMembershipModal from '../Modals/Membership/DeleteMembershipModal';
...
const TableMembership: React.FC<TableMembershipProps> = ({ searchedData }) => {
  // State variables for managing  data, UI state, pagination, and modal visibility.
  const [data, setData] = useState<Membership[]>([]);
  const [refreshData, setRefreshData] = useState<boolean>(true);
  const [showCreateModal, setShowCreateModal] = useState<boolean>(false);
  const [showDeleteModal, setShowDeleteModal] = useState<boolean>(false);
  const [deleteMembershipId, setDeleteMembershipId] = useState<number>();
  const [createMembership, setCreateMembership] = useState<Membership>(
    {
      Name_Person: '',
      Identify_Person: '',
      Date_Payment: '',
  ...
  // Define the endpoint for fetching membership data
  const endpoint = getEndpoint('memberships');
  ...
  const filteredData = useMemo(() => {
    ...
    return paginatedData.filter(membership =>
      membership.Identify_Person.toLowerCase().includes(searchTerm) ||
      membership.Name_Person.toLowerCase().includes(searchTerm) ||
      membership.Date_Payment.slice(0, 10).toLowerCase().includes(searchTerm) ||
      membership.Date_Expiration.slice(0, 10).toLowerCase().includes(searchTerm) ||
      membership.Type.toString().includes(searchTerm)
    );
  ...
  return (
    <>
      <Breadcrumb pageName="Membresías" />
      ...
      <div className="rounded-sm border border-stroke bg-white px-5 pt-6 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
        <div className="flex justify-end">
          ...
          {(currentUser.ID_Role !== 2 && currentUser.ID_Role !== 4) && (
                ...
                filteredData.length > 0 ? (
                  filteredData.map((item, key) => (
                    <tr key={key}>
                      ...
                      <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                        <p className="text-black dark:text-white">
                          {dateFormatted(item.Date_Payment.slice(0, 10), "cr")}
                        </p>
                      </td>
                      <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                        <p className="text-black dark:text-white">
                          {dateFormatted(item.Date_Expiration.slice(0, 10), "cr")}
                        </p>
                      </td>
                      <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                        <p className="text-black dark:text-white">
                      ...
                      <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                        <div className="flex items-center space-x-3.5">
                          ...
                          {(currentUser.ID_Role !== 2 && currentUser.ID_Role !== 4) && (
                            <button
                              onClick={() => { if (item.ID_Membership) handleOpenDeleteModal(item.ID_Membership) }}
