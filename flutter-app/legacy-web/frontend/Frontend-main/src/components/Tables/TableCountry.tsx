...
import CreateCountryModal from '../Modals/Country/CreateCountryModal';
import DeleteCountryModal from '../Modals/Country/DeleteCountryModal';
import UpdateCountryModal from '../Modals/Country/UpdateCountryModal';
...
const TableCountry: React.FC<TableCountryProps> = ({ searchedData }) => {
	...
	const [updateCountry, setUpdateCountry] = useState<Country | null>(null);
	const [deleteCountryId, setDeleteCountryId] = useState<number>();
	const [createCountry, setCreateCountry] = useState<Country>({
		Name: '',
		Cod_Reg: ''
	});
	...
	useEffect(() => {
		...
		const fetchData = async () => {
			try {
				const result = await getFetch('countries');
				console.log(result);
	...
	const filteredData = useMemo(() => {
		...
		return paginatedData.filter(country =>
			country.Name.toLowerCase().includes(searchTerm) ||
			country.Cod_Reg.toLowerCase().includes(searchTerm)
		);
	...
	const handleOpenUpdateModal = (country: Country) => {
										...
										<tr key={key}>
											...
											<td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
												<div className="flex items-center space-x-3.5">
													...
													<button
														onClick={() => { if (item.ID_Country) handleOpenDeleteModal(item.ID_Country) }}
